...
from torch_geometric.datasets import Amazon, Coauthor, CitationFull, Planetoid
...
def main(args):
    ...
    if args.dataset in ['Cora_CF', 'Cora_ML_CF', 'CiteSeer_CF', 'DBLP_CF', 'PubMed_CF']:
        ...
        print('1'*10)
        print(' args.dataset[:-3]', args.dataset[:-6])
        dataset = Planetoid(path, args.dataset[:-6])
        print('2'*10)
    ...
    if task == 'classification':
        ...
        # split_res = np.split(idx, [int(0.8 * len(idx)), int(0.1 * len(idx)), len(idx)])
        # print('split_res', split_res)
        # train_idx, valid, calib_test = split_res[0], split_res[1], split_res[2]
        
        total_len = len(idx)
        train_end = int(0.8 * total_len)  # End of training set
        valid_end = train_end + int(0.1 * total_len)  # End of validation set      
        train_idx, valid, calib_test = np.split(idx, [train_end, valid_end])
    ...
    #     num_nodes = len(y)
    #     # Adapting from your function: Calculating development set size considering min_num_test
    #     min_num_test = 100  # Ensuring minimum size for the test set
    #     num_development = min(1500, num_nodes - min_num_test)  # or adjust dynamically as in your function
    #     print('num_development', num_development, 'num_nodes', num_nodes)
    #     development_idx = idx[:num_development]
    #     calib_test = idx[num_development:]

    #     train_idx = []
    #     valid = []

    #     # Adapting class-balanced selection within the development set for training
    #     rnd_state = np.random.RandomState(args.data_seed)
    #     for c in range(int(np.max(y)) + 1):
    #         class_idx = development_idx[np.where(y[development_idx] == c)[0]]
    #         num_per_class = min(20, len(class_idx))  # Adapting num_per_class, ensuring not to exceed available class samples
    #         if num_per_class > 0:
    #             class_train_idx = rnd_state.choice(class_idx, num_per_class, replace=False)
    #             train_idx.extend(class_train_idx)

    #     # The remaining nodes in the development set are used for validation
    #     train_idx = np.array(train_idx)
    #     valid = np.array([i for i in development_idx if i not in train_idx])


    # Convert indices to boolean masks
    # num_nodes = len(y)
    # data.train_mask = np.zeros(num_nodes, dtype=bool)
    # data.valid_mask = np.zeros(num_nodes, dtype=bool)
    # data.calib_test_mask = np.zeros(num_nodes, dtype=bool)
    ...
    print(f"rations: train {sum(data.train_mask)}, val {sum(data.valid_mask)}, test {sum(data.calib_test_mask)}")
    # asdf
    ...
    for run in tqdm(range(args.num_runs)):
        ...
        (train_acc, val_acc, test_acc), predictions_test_me = test(best_model, data, alpha, tau, target_size, size_loss = False)
        result_this_run['gnn'] = {}
        ...
        if args.bnn:
            ...
            for epoch in range(1, args.epochs + 1):  
                ...
                # print('tmp_test_calib_acc', tmp_test_calib_acc)
                # print('pred', pred)
            ...
            prediction_classes = best_pred.argmax(dim=-1)
            out_softmax = F.softmax(best_pred, dim =1)
            print('out_softmax', out_softmax)
            accs = []
            for mask in [data.train_mask, data.valid_mask, data.calib_test_mask]:
                if task == 'classification':
                    accs.append(int((prediction_classes[mask] == data.y[mask]).sum()) / int(mask.sum()))
                else:
                    accs.append(pearsonr(best_pred[mask].detach().cpu().numpy().reshape(-1), 
                                        data.y[mask].detach().cpu().numpy().reshape(-1))[0])
            
            print('accs, train, val, test', accs)
            ...
            for entropy_threshold in entropy_thresholds:
                ...
                acc, _ = accuracy_with_entropy_threshold(out_softmax, data.y, entropy_threshold, mask_data=data.calib_test_mask)
                all_accuracies[str(entropy_threshold)].append(acc)
                print(f'Entropy Threshold: {entropy_threshold}', 'Test accuracy:', acc)                
