...
from gn_sde import BayesianGCN
...
def main(cmd_opt):
  ...
  def initialize_bayesian_gcn_model(device=device, edge_index=edge_index, num_feats=num_features, hidden_dim=hidden_dim, num_classes=num_classes, dropout=0):
    model = BayesianGCN(edge_index=edge_index, in_feats=num_feats, out_feats=num_classes, activation=F.softplus, dropout=dropout, opt=opt).to(device)
    return model
  ...
  model = initialize_bayesian_gcn_model(device, edge_index, num_features, hidden_dim, num_classes)
  ...
  prob_var , prob_mean, _ = mc_sample_stats(best_model, data.x, n_samples=20, pos_encoding=pos_encoding)
  prob_mean_cpu = prob_mean.detach().cpu().numpy()
  prob_var_cpu = prob_var.detach().cpu().numpy()
  ...
  save_probabilities_and_labels(prob_mean_cpu, prob_var=prob_var_cpu, model_name='bgcn', base_directory=base_directory, task_index=task_index, labels=data.y.detach().cpu().numpy(), is_ODD=opt['OOD'])
