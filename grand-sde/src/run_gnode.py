...
from gn_sde import GraphNeuralODE
...
def main(cmd_opt):
  ...
  def initialize_graph_neural_ode(device, edge_index, num_features, hidden_dimension, num_classes):
    ...
    graph_neural_ode= GraphNeuralODE(ode_func=gnn_f, in_net=gcn_in, out_net=gcn_out, method='rk4', device=device, opt=opt).to(device)

    return graph_neural_ode
  ...
  save_probabilities_and_labels(prob_mean_cpu, prob_var=prob_var_cpu, model_name='gnode', base_directory=base_directory, task_index=task_index, labels=data.y.detach().cpu().numpy(), is_ODD=opt['OOD'])
