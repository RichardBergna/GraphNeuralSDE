...
def save_probabilities(prob_mean, model_name, base_directory, task_index):
...
def main(cmd_opt):
  ...
  """saves the probilities of each of the class"""
  _ , prob_mean, _ = mc_sample_stats(best_model, data.x, n_samples=10, pos_encoding=pos_encoding)
  prob_mean_cpu = prob_mean.detach().cpu().numpy()
  # Save the probabilities
  base_directory = f'resutls_prob/{args.dataset}'
  # rob_mean,model_name, base_directory, task_i
  save_probabilities(prob_mean_cpu, model_name='grand-sde', base_directory=base_directory, task_index=task_index)
...
if __name__ == '__main__':
  ...
  model_sde_name = 'GRAND-SDE_sigma{}_rtol{}_t{}'.format(sigma_name, rtol_name, sde_t_name)
  
  if opt['planetoid_split'] or opt['use_80_10_10_split']:
    model_sde_name += '_planetoid_split'
