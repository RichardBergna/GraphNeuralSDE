...
#     gcn_models = [initialize_gnn().to(device) for _ in range(number_of_ensemble)]
#     for model in gcn_models:
#         model = train(model, criterion, data, steps, verbose_step, kl_scheduler=kl_scheduler, accuracy_func=accuracy_func, is_stochastic=is_stochastic)
...
def mc_sample_stats(model, feat, n_samples=1, pos_encoding=None):
    ...
    samples_tensor = torch.stack(samples) # to keep it consistent we keep the samples mean and var as torch tensors.
...
def ensemble_stats(models, feat, pos_encoding=None):
    samples = []  
    samples_entropy = []
    for model in models:  
        pred, _ = model(feat)
...
def evaluate_ensemble_models(models, data, feat, entropy_threshold=np.inf, pos_encoding=None):
  with torch.no_grad():
    _, samples_mean, samples_entropy = ensemble_stats(models, feat, pos_encoding=pos_encoding)
    # acc, num_of_predictions = accuracy_with_entropy_threshold(samples_mean, samples_entropy, Y, entropy_threshold=entropy_threshold, mask_data=mask_data)
    accs = []
    for _, mask in data('train_mask', 'val_mask', 'test_mask'):
      acc, _ = accuracy_with_entropy_threshold(samples_mean, samples_entropy, data.y, entropy_threshold=entropy_threshold, mask_data=mask)
...
def test_ensembles(models, data, pos_encoding=None, opt=None, entropy_threshold=np.inf, n_samples=1):  # opt required for runtime polymorphism
  feat = data.x
  if models[0].opt['use_labels']:
    feat = add_labels(feat, data.y, data.train_mask, models[0].num_classes, models[0].device)
    
  accs = evaluate_ensemble_models(models, data, feat, entropy_threshold=entropy_threshold, pos_encoding=pos_encoding)
  return accs
...
def test_OGB(model, data, pos_encoding, opt):
  ...
  out, _ = model(feat)
  # I think we are not using softmax so we are not yielding any probabilities.
  # Once we have the prob just return them and save them in a file.
  # Repeat the experiment 100 times for all models   
...
def save_probabilities_and_labels(prob_mean,model_name, prob_var, base_directory, task_index, labels, is_ODD=False):
    ...
    if is_ODD:
      model_name = f'{model_name}_ODD'
      print("new_name: ", model_name)
    # Construct the full file path
    file_path = f'{base_directory}/{model_name}_prediction_probabilities_{task_index}.npz'

    print('file_path 1', file_path)
     # Check if the file already exists
    counter = 1
    while os.path.exists(file_path):
        file_path = f'{base_directory}/{model_name}_prediction_probabilities_{task_index}_(%d).npz' % counter
        counter += 1
    print('file_path 2', file_path)
        
    # If prob_mean is a PyTorch tensor, transfer it to CPU and convert to NumPy array
    if hasattr(prob_mean, 'detach'):
        prob_mean_cpu = prob_mean.detach().cpu().numpy()
    else:
        prob_mean_cpu = prob_mean
        
    if hasattr(prob_var, 'detach'):
        prob_var_cpu = prob_var.detach().cpu().numpy()
    else:
        prob_var_cpu = prob_var

    # Save the probabilities to the determined file path
        # If labels is a PyTorch tensor, transfer it to CPU and convert to NumPy array
    if labels is not None:
        if hasattr(labels, 'detach'):
            labels_cpu = labels.detach().cpu().numpy()
        else:
            labels_cpu = labels
        # Save both probabilities and labels
        np.savez_compressed(file_path, prob_mean=prob_mean_cpu, prob_var=prob_var_cpu, labels=labels_cpu)
    else:
        raise f"Error labels == {labels}"
        # Save only probabilities
        # np.savez_compressed(file_path, prob_mean=prob_mean_cpu)

    return file_path
          
def main(cmd_opt):

  number_epochs_before = cmd_opt['epoch']
  try:
  ...
  opt['epoch'] = number_epochs_before



  dataset = get_dataset(opt, f'{ROOT_DIR}/data', opt['not_lcc'], opt['OOD'])
  ...
  def initialize_gnn(device, edge_index, num_features, hidden_dimension, num_classes):
    gcn_model = MyGCN2(edge_index=edge_index, in_feats=num_features, n_dimension=hidden_dimension, out_feats=num_classes, activation=None, dropout=0, opt=opt).to(device)
    return gcn_model
  ...
  # model = initialize_gnn(device, edge_index, num_features, hidden_dim, num_classes)
   # Step 1: Train individual models
  number_of_ensemble = 5
  gcn_models = [initialize_gnn(device, edge_index, num_features, hidden_dim, num_classes) for _ in range(number_of_ensemble)] 
  
  print('gcn_models', gcn_models)
  
  
  if not opt['planetoid_split'] and opt['dataset'] in ['Cora','Citeseer','Pubmed']:
    dataset.data = set_train_val_test_split(np.random.randint(0, 1000), dataset.data, num_development=5000 if opt["dataset"] == "CoauthorCS" else 1500, OOD=opt['OOD'])
  ...
  for i, model in enumerate(gcn_models):
    ...
    for epoch in range(1, opt['epoch']):
      ...
      if opt['OOD']:
        ...
        data.train_mask = data.train_mask & (data.y < num_classes - 1)
        train_last_class_count_after = (data.train_mask & (data.y == num_classes - 1)).sum().item()
        test_last_class_count_after = (data.test_mask & (data.y == num_classes - 1)).sum().item()
        
        print(f"After OOD adjustment: Training instances of last class: {train_last_class_count_after}")
        print(f"After OOD adjustment: Testing instances of last class: {test_last_class_count_after}")
    
        loss = train(model, optimizer, data, pos_encoding)
      
        tmp_train_acc, tmp_val_acc, tmp_test_acc = this_test(model, data, pos_encoding, opt)

        best_time = model.opt['time']
  ...
  prob_var , prob_mean, _ = ensemble_stats(best_models, data.x, pos_encoding=pos_encoding)
  prob_mean_cpu = prob_mean.detach().cpu().numpy()
  prob_var_cpu = prob_var.detach().cpu().numpy()
  # Save the probabilities
  base_directory = f'resutls_prob/{args.dataset}'
  # rob_mean,model_name, base_directory, task_i
  if opt['OOD']:
    base_directory = f'resutls_prob/OOD_{args.dataset}'
    
  save_probabilities_and_labels(prob_mean_cpu, prob_var=prob_var_cpu, model_name='ensemble', base_directory=base_directory, task_index=task_index, labels=data.y.detach().cpu().numpy(), is_ODD=opt['OOD'])
  ...
  for entropy_threshold in entropy_thresholds:
    ...
    _, _, test_acc = these_test(best_models, data, pos_encoding, opt, entropy_threshold, n_samples=10)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--OOD', action='store_true',
                      help='Performs Out of distribution detection, by not including the last class in the trainig data.')
