...
def main(cmd_opt):
  ...
  class DriftNN(nn.Module):
    def __init__(self, edge_index, hidden_dimension, num_of_layers, device):
        super(DriftNN, self).__init__()
        self.layers = nn.ModuleList()
        # Always add the first layer
        self.layers.append(MyGCN(edge_index=edge_index, in_feats=hidden_dimension + 1, 
                                 out_feats=hidden_dimension, activation=nn.Softplus(), dropout=0.2))
        
        # Add additional layers if num_of_layers > 1
        for l_num in range(1, num_of_layers):
            print('Adding layer', l_num)
            self.layers.append(MyGCN(edge_index=edge_index, in_feats=hidden_dimension, 
                                     out_feats=hidden_dimension, activation=nn.Softplus(), dropout=0.2))
            if l_num == num_of_layers-1:
                print('Adding final layer', l_num)
                self.layers.append(MyGCN(edge_index=edge_index, in_feats=hidden_dimension, 
                                     out_feats=hidden_dimension, activation=None, dropout=0.2))
  ...
  def initialize_graph_neural_sde(device, edge_index, num_features, hidden_dimension, num_classes):
    ...
    gnn_f = DriftNN(edge_index=edge_index, hidden_dimension=hidden_dimension, num_of_layers=opt['sde_num_layers'], device=device)
    ...
    # Initialize the Graph Neural SDE model opt
...
if __name__ == '__main__':
 ...
 # new
  ...
  parser.add_argument('--sde_t', type=float, default=1.0, help="final time constant for the SDE solver")
  parser.add_argument('--sde_num_layers', type=int, default=2, help="Number of layers for the drift function in the SDE")
  ...
  sde_num_layers = str(opt['sde_num_layers'])
  model_sde_name = 'oversmoothing_{}_TRUE_LGNSDE_sigma{}_rtol{}_t{}'.format(sde_num_layers, sigma_name, rtol_name, sde_t_name)
